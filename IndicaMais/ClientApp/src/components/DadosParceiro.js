...
import style from './DadosParceiro.module.scss';
...
function DadosParceiro({ id, fechar }) {
    ...
    const atualizarQtdIndicacoes = async (idParceiro) => {
        setQtdIndicacoes({
            total: await fetch.contarIndicacoesId(idParceiro, false),
            fechadas: await fetch.contarIndicacoesId(idParceiro, true)
        });
    }
    ...
    useEffect(() => {
        const buscarDados = async () => {
            if (id) {
                ...
                setDados({ lista: parceiro.tipo === 0 ? itens.transacoes : itens.indicacoes, temMais: itens.temMais });
            } else {
            ...
            <>
                <div className={style.dados}>
                    ...
                    <div className={style.nome}>
                        ...
                        {parceiro.tipo === 0 ?
                            <>
                                <div className={style.informacoes}>
                                    <p title="Indicado por"><FontAwesomeIcon icon={faHandPointRight} /> {parceiro.indicadoPor != null ? parceiro.indicadoPor : "[Não foi indicado]"}</p>
                        ...
                        :
                            <>
                                <h3 className={style.statusparceiro}><FontAwesomeIcon icon={faGem} title="Parceiro" /> Parceiro do seu escritório</h3>
                                <div className={style.informacoes}>
                                    <p title="CPF"><FontAwesomeIcon icon={faIdCard} /> {parceiro.cpf ? <InputMask mask="999.999.999-99" disabled value={parceiro.cpf} /> : "[Sem CPF]"}</p>
                                    <p title="Número de telefone"><FontAwesomeIcon icon={faPhone} /> <InputMask mask="(99) 99999-9999" disabled value={parceiro.telefone} /></p>
                                </div>
