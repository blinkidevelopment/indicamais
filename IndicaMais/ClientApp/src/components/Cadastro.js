                    ...
                    if (response != null) {
                        ...
                        if (status === 200) {
                            ...
                            if (resposta.status === true) {
                                setMensagem("Você já possui cadastro. Clique no botão abaixo para entrar com seu CPF e senha");
                                setPossuiCadastro(true);
                            } else if (resposta.statusCRM === true && resposta.status === false) {
                                setStatusCadastro({ ...statusCadastro, exibirCamposSeguintes: true });
                            }else {
    ...
    async function enviarSolicitacao() {
        ...
        if (valCpf === true && valSenha.confirmacao === true && cpf !== null) {
            ...
            if (celular === '' || celular.length < 11) {
                ...
                if (statusCadastro.cadastroIndicado) {
                    resultado = await fetch.atualizarDadosParceiro(document.getElementById('celular').value.replace(/[()\-_ ]/g, ''), cpf.replace(/[.\-_]/g, ''), senha, confirmacao);
                } else {
                    resultado = await fetch.cadastrarParceiro(document.getElementById('nome').value, document.getElementById('celular').value.replace(/[()\-_ ]/g, ''), cpf.replace(/[.\-_]/g, ''), senha, confirmacao);
                }
        ...
        <div className={style.cadastro}>
            <div className={style.cadastrocard}>
                ...
                <div className={style.formulario}>
                    ...
                    :
                        <div className={style.opcoescadastro}>
                            <button onClick={() => setStatusCadastro({exibir: true, cadastroCliente: true, cadastroIndicado:false})}>Já sou cliente</button>
